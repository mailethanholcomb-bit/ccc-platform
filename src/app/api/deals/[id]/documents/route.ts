...
function generateSummaryPdf(deal: DealWithRelations): NextResponse {
  ...
  const summary = ms.summaryJson as Record<string, unknown>;
  const snap = (summary.dealSnapshot ?? {}) as Record<string, string>;
  const fin = (summary.financialHealth ?? {}) as Record<string, string>;
  const flags = (summary.redFlags ?? []) as Array<{ flag: string; severity: string }>;
  const greens = (summary.greenFlags ?? []) as string[];
  const steps = (summary.nextSteps ?? []) as string[];
...
function generateCaseStudy(deal: DealWithRelations): NextResponse {
  ...
  const phases = [
    { title: "Phase 1: Industry Research", data: pr.phase1Json },
    { title: "Phase 2: Digital Footprint", data: pr.phase2Json },
    { title: "Phase 3: Distress Signals", data: pr.phase3Json },
    { title: "Phase 4: Weighted Scorecard", data: pr.phase4Json },
  ];
...
<p>Verdict: <strong>${(pr.overallVerdict ?? "N/A").toUpperCase()}</strong></p>
<p>Score: ${pr.weightedScore ?? "N/A"}/100</p>

${phases.map((p) => `<h2>${p.title}</h2><pre>${p.data ? JSON.stringify(p.data, null, 2) : "Not available"}</pre>`).join("\n")}
...
async function generateBenchmarkXlsx(deal: DealWithRelations): Promise<NextResponse> {
  ...
  const screeningData = br.screeningJson as { traits?: Array<Record<string, string>> } | null;
  ...
  const benchData = br.industryBenchmarkJson as Record<string, unknown> | null;
  ...
  const sensitivityData = br.sensitivityJson as { matrix?: Array<Record<string, unknown>>; keyInsights?: string[] } | null;
...
async function generateAnalyzerXlsx(deal: DealWithRelations): Promise<NextResponse> {
  ...
  // Tab 2: Projections
  const projections = ar.projectionsJson as Array<Record<string, unknown>> | null;
  ...
  const strategy = ar.strategyJson as Array<Record<string, unknown>> | null;
  ...
  // Tab 4: Grade Details
  const gradeDetails = ar.gradeDetailsJson as Array<Record<string, unknown>> | null;
...
type DealWithRelations = NonNullable<
  Awaited<
    ReturnType<
      typeof prisma.deal.findUnique<{
        include: {
          dealAnalyzerResult: true;
          protocolResult: true;
          benchmarkResult: true;
